...
RSpec.describe 'Micropub Server Implementation Report - Media Endpoint', type: :request do
  ...
  describe '700 - Upload a jpg to the Media Endpoint' do
    ...
    it 'returns a Location header with the URL of the new asset' do
      expect(response.headers['Location']).to eq(new_asset.file_url)
    end

    it 'creates an orphan asset' do
      expect(new_asset).not_to be_nil
    end

    it 'sets the mime type for the asset' do
      expect(new_asset.mime_type).to eq('image/jpeg')
    end
  end
  ...
  describe '701 - Upload a png to the Media Endpoint' do
    ...
    it 'sets the mime type for the asset' do
      expect(new_asset.mime_type).to eq('image/png')
    end
  end
  ...
  describe '702 - Upload a gif to the Media Endpoint' do
    ...
    it 'sets the mime type for the asset' do
      expect(new_asset.mime_type).to eq('image/gif')
    end
  end
end
